...
on:
  ...
  workflow_dispatch:
    inputs:
      ...
      #
      # The dispatch form also shows gh-aw's own `aw_context` field above this
      # one. Leave it empty — it is parsed as JSON, and anything else there
      # kills the activation job before the agent starts.
      issue_number:
        description: "Issue number only, e.g. 95. Empty drains the backlog by label as usual. Leave the aw_context field above blank."
        required: false
        type: string
