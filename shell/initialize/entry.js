...
import { loadDebugger, mountApp, storeInit } from '@shell/initialize/entry-helpers';
...
extendApp(Vue)
  .then((appPartials) => mountApp(appPartials, Vue))
  .then(({store, app}) => storeInit(store, app.context))
  .catch(errorHandler); // eslint-disable-line no-undef
